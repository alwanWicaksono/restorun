...
class Controller {
  ...
  static async deleteItem(req, res, next) {
    try {
      const itemFound = await Item.findByPk(req.params.itemId);
  ...
  static async editItem(req, res, next) {
    try {
      ...
      let findItem = await Item.findByPk(req.params.itemId);
      if(!findItem) throw {name: "Item Not Found"}
      await Item.update({name, description, price, imgUrl, CategoryId}, {where: {id: req.params.itemId}})
